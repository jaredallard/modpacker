...
module.exports = {
  ...
  installModpack: async url => {
    ...
    let tmpDir = path.join(os.tmpdir(), "modpacker")
    ...
    if (!await fs.pathExists(path.join(url, "modpack.yaml"))) {
      ...
      log.info('modpack is already unpacked, not extracting')

      // nexe will insert files that don't actually exist in cwd(), so we use the hardcoded dirs
      for (const folder of folders) {
        await fs.copy(path.join(url, folder), path.join(tmpDir, folder))
      }
    }

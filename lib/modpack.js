...
module.exports = {
  ...
  installModpack: async url => {
    ...
    log.info('installing modpack "%s" v%s', config.name, config.version)

    const forgeProfile = `${config.minecraft.version}-forge${config.minecraft.version}-${config.forge.version}`
    if (config.forge.version) {
      if (!await fs.pathExists(path.join(minecraftHome, `versions/${forgeProfile}`))) {
    ...
    if (!profiles.profiles[config.name]) {
      ...
      profiles.profiles[config.name] = {
        ...
        lastVersionId: config.forge.version ? forgeProfile : config.minecraft.version,
        icon: 'Furnace',
        type: 'custom',
      }
